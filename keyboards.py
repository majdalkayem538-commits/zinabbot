...
def persistent_main_keyboard():
    ...
    return ReplyKeyboardMarkup(
        ...
        input_field_placeholder="اختر من القائمة أو اكتب رسالتك…",
    )
...
def main_menu_keyboard(has_approved: bool, session_valid: bool):
    ...
    if has_approved:
        ...
        rows.append([InlineKeyboardButton("🔄 تغيير PIN", callback_data="menu_security")])
...
def years_keyboard(selected_subjects: list[str]):
    ...
    rows.append([
        InlineKeyboardButton(
            f"💳 متابعة الدفع ({len(selected_subjects)} مادة - {total}$)",
            callback_data="menu_checkout",
        )
    ])
...
def year_subjects_keyboard(year_key: str, selected_subjects: list[str]):
    ...
    if year:
        for subject_key, subject in year.get("subjects", {}).items():
            ...
            rows.append([
                InlineKeyboardButton("ℹ️ تفاصيل المادة", callback_data=f"subject_info|{subject_key}")
            ])
...
def orders_keyboard(has_approved: bool):
    rows = [[InlineKeyboardButton("🛒 السلة والدفع", callback_data="menu_checkout")]]
    if has_approved:
...
def lectures_keyboard(subject_key: str):
    ...
    if subject:
        for lecture in subject.get("lectures", []):
            ...
            rows.append([
                InlineKeyboardButton(label, callback_data=f"lecture|{subject_key}|{lecture['key']}")
            ])
