...
from helpers import (
    is_admin, set_state, get_state, clear_state, get_years, flatten_subjects,
...
async def admin_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "admin_pending":
        ...
        for row in rows:
            lines.append(f"🧾 {row[1] or '-'} | 👤 {row[0]} | 📚 {row[3] or '-'} | 💳 {row[2] or '-'} | 📌 {row[5]} | 🕒 {row[4]}")
        await query.message.reply_text("\n".join(lines))
        return
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "await_subject_price":
        ...
        await update.message.reply_text(f"✅ تم إضافة المادة بنجاح.\nالسنة: {year_key}\nالمفتاح: {subject_key}\nالاسم: {subject_title}\nالسعر: {price}$")
        return
