...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data in ("menu_start", "menu_home"):
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row["form_step"] == "payer_full_name":
        cursor.execute(
            "UPDATE users SET cash_full_name=?, form_step='payer_student_number' WHERE user_id=?",
            (text, user.id),
        )
        conn.commit()
    ...
    if row["form_step"] == "payer_student_number":
        cursor.execute(
            "UPDATE users SET cash_student_number=?, form_step='payer_phone' WHERE user_id=?",
            (text, user.id),
        )
        conn.commit()
